...
%   [SH] - 12/06/13:  Enhanced 'gooddata' to include pre- and post-
...
% 
    gooddata.before = length(find(vec.eyes ~=0))/length(vec.eyes);
    newname = {'x', 'y', 'd', 'v', 'p'};
...
%       much of our data is usable.
    gooddata.after = length(find(~isnan(x)))/length(x);
