...
try
    [subnum, substr, subraw] = xlsread([Direct, '\EXCEL\' StudyName ' SUMMARY.xlsx'], 'Participants');
catch
...
[filename,currenttime] = updatefileinfo(Direct, StudyName, 1); 

if ~isfield(rawdata, 'version')
    rawdata.version.CreationDate = currenttime;
    rawdata.version.EditLog = {currenttime, VersionNumber, ChangedSubs};
else
    rawdata.version.EditLog = [rawdata.version.EditLog; {currenttime, VersionNumber, ChangedSubs}];
end
rawdata.version.LastEdit = currenttime;
