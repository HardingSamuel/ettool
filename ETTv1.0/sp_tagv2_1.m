function [sp_index_final] = sp_tagv2_1(x,y,d,velo,sr,varargin)
...
xqi = unique(cell2mat(arrayfun(@(x) xqi(x):xqi(x)+sp_windowsize, [1:length(xqi)], 'uni', 0)));
if ~isempty(xqi)
    xq_begin = [xqi(1), xqi(find(diff(xqi)>1)+1)]; %location of beginning of sp
    xq_end = [xqi(find(diff(xqi)>1)), xqi(end)]; %location of end of sp
    xq_thresh = xq_end - xq_begin + 1 >= 0; %index of sp beginnings where duration >= threshold
...
yqi = unique(cell2mat(arrayfun(@(y) yqi(y):yqi(y)+sp_windowsize, [1:length(yqi)], 'uni', 0)));

if ~isempty(yqi)
    yq_begin = [yqi(1), yqi(find(diff(yqi)>1)+1)];
    yq_end = [yqi(find(diff(yqi)>1)), yqi(end)];
    yq_thresh = yq_end - yq_begin + 1 >= 0;
...
for arfill = 1:length(whatson.Begindices)
    rectangle('pos',[st*whatson.Begindices(arfill), ax(3), 2, ax(4)-ax(3)],'edgecolor',[0 0 1])
end
xlabel('Time (ms)')
...
if ~size(data.sortdat,1)==0
    editwin = figure('position', [ 809   373   700   625], 'numbertitle', 'off', 'name', 'Edit SP Epochs',  'color', [.7 .7 .7], ...
...
%                     data.epochs.x(xrow,2)
    ...
    function PhaseFig(auditmain_menu1_2, eventdata, whatson)
        phasefig = figure('position', [ 600   372   200    100], 'numbertitle', 'off', 'name', 'Phase Information', 'menubar', 'none', 'color', [.7 .7 .7]);
        phasedatnames = uicontrol('style', 'text', 'pos', [10 10 85 90], 'string', whatson.Names);
        phasedatindex = uicontrol('style', 'text', 'pos', [105 10 85 90], 'string', st*whatson.Begindices);
    end
