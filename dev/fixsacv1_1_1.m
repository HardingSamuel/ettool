function [fixinfo, sacinfo, pointinfo] = fixsacv1_1_1(x,y,d,sr,gapdata,fff)
%FixDetectDev
%
...
minsactime = 40; %saccades must last at least this long (ms) or they cannot have been true saccades and are likely noise
...
if length(fix_keep_final)>0
    ...
    fixinfo.onsets = fix_keep_begin_final;
else
    fixinfo.nfix = [];
    fixinfo.durations = [];
    fixinfo.onsets = []; 
end
if length(sac_keep_final)>0
    sacinfo.nsac = length(sac_keep_begin_final);
    sacinfo.durations = sac_durations;
    sacinfo.onsets = sac_keep_begin_final;
else
    sacinfo.nsac = [];
    sacinfo.durations = [];
    sacinfo.onsets = [];
...
pointinfo.bayes = still_unknown_final;
