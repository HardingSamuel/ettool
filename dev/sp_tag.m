function [sp_index_final] = sp_tag(x,y,d,velo,sr,varargin)
...
if ~isempty(xqi)
    ...
    xq_end = [xqi(find(diff(xqi)>1)), length(xqi)]; %location of end of sp
...
if ~isempty(yqi)
    ...
    yq_end = [yqi(find(diff(yqi)>1)), length(yqi)];
...
%% Functions
    ...
    function BeginEdits(auditmain_menu2_2, eventdata)
        editwin = figure('position', [100 350 700 625], 'numbertitle', 'off', 'name', 'Edit SP Epochs',  'color', [.7 .7 .7], ...
        ...
        nextsp = uicontrol('style', 'pushbutton', 'string', 'Next', 'callback', {@time_forward, backsp}, 'position', [600 20 80 30]);
