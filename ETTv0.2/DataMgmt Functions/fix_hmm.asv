function [fixinfo] = fix_hmm(velo,fixinfo,fixdetset,states,sr)
%
% fix_hmm
...
switch mode
    case 1
        counts = arrayfun(@(state) hist(velo(states==state),1:ceil(max(max(velo))/binsize)),1:3,'uni',0);
    ...
    case 2
        for trinum = 1:size(states,1)
            counts{trinum} = arrayfun(@(state) hist(velo(trinum,states(trinum,:)==state),0:ceil(max(max(velo))/binsize)),1:3,'uni',0);
            velodist{trinum} = arrayfun(@(state) cell2mat(counts{trinum}(state))/sum(cell2mat(counts{trinum}(state))),1:3,'uni',0);
        end
        
end

estim_states(isnan(velo)) = 999;
