...
trialswithfix = find(arrayfun(@(tri) any(fixinfo.ivt.isfix(tri,:)==1),1:size(fixinfo.ivt.isfix)));
fixinfo.ivt.fixbegin(sub2ind([size(fixinfo.ivt.isfix,1),size(fixinfo.ivt.isfix,2)],trialswithfix,arrayfun(@(tri) find(fixinfo.ivt.isfix(tri,:)==1,1,'first'),trialswithfix))) = 1;
fixinfo.ivt.fixend(sub2ind([size(fixinfo.ivt.isfix,1),size(fixinfo.ivt.isfix,2)],trialswithfix,arrayfun(@(tri) find(fixinfo.ivt.isfix(tri,:)==1,1,'last'),trialswithfix))) = 1;
