...
%%
% some feedback to the user, context sensitive
switch mode
    case 0
        text_done = 'Finished';
    case 1
...
% if we gave it some existing settings when we called this function,
% replace the defaults with these
...
% convert some numerical values to strings
...
%       when used clicks different bubble, change the filter type.
%       currently disabled as only SGolay filter is implemented
        curr_fil = get(eventdata.NewValue,'UserData');
    end

    function filt_edit(~,~)
%       filters each have some parameters; based on which filter is
%       currently selected, bring up a box inside the figure to adjust
%       these
...
%         based on each filter, show different UI elements
        switch curr_fil
            case 1
        ...
        function slide_text(~,~)
%           format the text on the slider in the interface
            text_ex = '';
...
%           when user hits the "return" button after editing an individual
%           filter's settings, send these updated values back 
...
%       once the user is done viewing or changing these settings, return
%       the new values to the caller
...
% ths may be unnecessary, but goal is to enforce certain windows to be on
% top of each other and not allow background windows to execute while
% manipulations in this window are still pending
uiwait(PPFig)
try
close(PPFig)
end
